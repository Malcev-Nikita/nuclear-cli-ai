...
- `src/core/` — ядро: `wake.py` (WakeMatcher: имя/bare/shutup/контекст,
  ...
  позавчера?» через `Skill.follow_up`; вырожденный ответ вида «имя_инструмента\n
  {json}» тоже чинится — вживую 2026-08-02), `texts.py`
...
- **Wake word без openWakeWord** (`src/core/wake.py`): под русское имя «Игорь» готовой
  ...
  ПОСЛЕ КАЖДОГО ОТВЕТА открывается второе окно (FOLLOWUP_SEC): короткая фраза,
  начатая с «а»/«и» (`WakeMatcher.is_follow_up`), принимается без имени — иначе
  «А в Питере?» после погоды уходило в «мимо» (вживую 2026-08-02), ведь
  уточнения агента жили за фильтром имени.
