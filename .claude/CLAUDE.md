...
- Этап 2: голос. STT ✅ собран (`voice.py`: sounddevice → энергетический VAD →
  ...
  во время озвучки `Speaker.say(text, mic)` сам читает блоки микрофона, меряет
  эхо своего голоса первые 0.4 с и обрывает речь (`sd.stop()`), если пользователь
  говорит громче эха в `BARGE_GAIN` раз ~0.25 с подряд. ВАЖНО: перед замером —
  `mic.flush()` и скип 0.2 с (вживую 2026-08-02: без этого эталон эха мерился по
  тишине из очереди, накопленной пока думал агент, и на длинных ответах ассистент
  перебивал сам себя — «звук не воспроизводится»). «замолчи/заткнись/помолчи»
