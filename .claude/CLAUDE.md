...
- **16 узких инструментов вместо сырых 4 мета-инструментов Nuclear MCP**
...
- **Wake word без openWakeWord** (`voice.py`): под русское имя «Игорь» готовой
  ...
  побочно и падежи «Игоря». Короткие управляющие команды (`BARE_COMMANDS` в
  voice.py: стоп/пауза/дальше/громче/…) принимаются без имени — только если
  фраза целиком состоит из команды; список расширять осторожно (сработает от
  чужого разговора/ТВ).
