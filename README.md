...
| `какая погода` / `погода в казани` | без LLM → wttr.in (интернет, без API-ключа) |
...
- **Узкие инструменты вместо сырого MCP**: модель выбирает из 13 понятных действий
...
сбрасывается, чтобы ассистент не слушал сам себя.
