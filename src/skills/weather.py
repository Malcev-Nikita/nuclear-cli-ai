...
# «на» перед днём необязательно: вживую сказали «погода НА завтра» (2026-08-02)
_WHEN = (rf"((?:на\s+)?(?:сегодня|завтра|послезавтра)|на\s+выходны\w+|в\s+выходные|{_WEEKDAY})")
_ASK = r"(?:погод\w*|прогноз\w*(?:\s+погоды)?)"  # «погода», «погоды», «прогноз»
...
class WeatherSkill(Skill):
    ...
    def rules(self) -> list[Rule]:
        return [
            ...
            Rule(rf"^(?:какая\s+|какой\s+)?(?:будет\s+)?{_ASK}\s+(?:будет\s+)?{_WHEN}"
                 r"(?:\s+в\s+(.+?))?\s*\??$",
            ...
            # (?!…) — «на завтра» не должно уехать в название города
            Rule(r"^(?:какая\s+)?(?:сейчас\s+)?погод\w*(?:\s+(?:сейчас|сегодня))?"
                 r"(?:\s+(?:в|на)\s+(?!завтра|послезавтра|выходн|сегодня)(.+?))?\s*\??$",
